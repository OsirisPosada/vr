...
namespace UnityEngine.XR.OpenXR.Features.Android
{
    ...
    public sealed class AndroidOpenXRFaceSubsystem : XRFaceSubsystem
    {
        ...
        /// <summary>
        /// Attempts to get whether or not face tracking is calibrated for the person wearing the headset.
        /// This call may fail if the underlying runtime does not support querying the calibration state.
        /// If face tracking is not calibrated, consider directing users to the face calibration flow in the operating system settings.
        /// </summary>
        /// <returns>A result indicating whether or not tracking is calibrated for the inward face.
        /// This indicator is only valid if the result is successful and otherwise may be uninitialized or default.</returns>
        public Result<bool> TryGetIsFaceTrackingCalibrated()
        {
            return ((AndroidOpenXRFaceProvider)provider).TryGetIsFaceTrackingCalibrated();
        }
        ...
        class AndroidOpenXRFaceProvider : Provider
        {
            ...
            public Result<bool> TryGetIsFaceTrackingCalibrated()
            {
                ...
                if (!OpenXRRuntime.IsExtensionEnabled(Constants.OpenXRExtensions.k_XR_ANDROID_face_tracking))
                {
                    Debug.LogError($"{Constants.OpenXRExtensions.k_XR_ANDROID_face_tracking} must be enabled to query for face calibration state.");
                    resultStatus = new XRResultStatus(XRResultStatus.StatusCode.ProviderUninitialized);
                    return new Result<bool>(resultStatus, default);
                }

                resultStatus = NativeApi.TryGetFaceCalibrationState(out var ptrFaceIsCalibratedOutput);
                ...
                return new Result<bool>(resultStatus, ptrFaceIsCalibratedOutput);
            }
            ...
            static class NativeApi
            {
                ...
                [DllImport(Constants.k_ARFoundationLibrary, EntryPoint = "UnityOpenXRAndroid_Face_TryGetFaceCalibrationState")]
                public static extern XRResultStatus TryGetFaceCalibrationState(
                    out bool ptrFaceIsCalibratedOutput);
            }
        }
    }
}
