...
### Changed
- Changed the minimum supported Android SDK version to API level 25 for Unity 6.3+ and level 26 for Unity 6.5+, matching the minimum supported SDK versions for each Unity version.
- Android XR: Performance Metrics and Hand Mesh Data features now being included as default in Android XR feature group.
- Moved `AndroidXRSettingsInitializer` asset file from the **Assets** folder to the **ProjectSettings** folder and renamed it to `AndroidXRSettingsManager.asset`.

### Fixed
- Fixed usages of APIs that were deprecated in AR Foundation 6.5 and Unity 6.3.
- Fixed internal build logic so that builds from the Unity 6.6 alpha now correctly contain their Android App Manifest entries.
- Fixed settings management singleton so it no longer modifies its underlying asset during import, preventing related warnings.

## [1.3.0] - 2026-04-06

### Added
- Added the `DimmingLevelChanged` event to [ARCameraFeature](xref:UnityEngine.XR.OpenXR.Features.Android.ARCameraFeature), which notifies subscribers when the global passthrough dimming level changes externally, such as through a physical button press or a system-level settings change.

### Changed
- Dimming now defaults based on the current blend mode (Opaque: maximum, AlphaBlend/Additive: minimum) unless explicitly set via the TryRequestGlobalDimmingLevel API. 

## [1.3.0-pre.2] - 2026-03-20

### Added
- Added support for Global Passthrough Dimming.

### Fixed
- Fixed build failure due to null reference exception in projects without configured optional features by pre-initializing the optionalFeatures array.

## [1.3.0-pre.1] - 2026-03-08

### Added
- Added the `TryGetIsFaceTrackingCalibrated` method to the [AndroidOpenXRFaceSubsystem](xref:UnityEngine.XR.OpenXR.Features.Android.AndroidOpenXRFaceSubsystem), which adds support for querying the face tracking calibration state.
- Added support for optional feature selection and generating the application manifest appropriately, including adding Spatial API version.

### Fixed
- Fixed the `AndroidXRHandMeshData` feature so that if it fails to initialize due to lack of permissions, it can automatically recover when hand tracking permission is later granted.

## [1.2.0] - 2025-12-22

### Added
- Added support for Composition Layers Passthrough Splash Screen (requires Composition Layers version 2.3.0+)

### Changed
- Changed the project validation rules to no longer show an extraneous validation warning regarding an upcoming feature.
- Updated ARF package dependency to 6.4.0

### Fixed
- Fixed the Foveated Rendering feature preventing Unity projects using it from appearing on the Play
  Store for devices without eye tracking - it uses the Eye Tracking permission for ETFR but
  gracefully degrades to FFR without permissions or hardware support.

## [1.2.0-pre.2] - 2025-11-13

### Changed
- Changing OpenXR Plugin dependency to version 1.16.0.

### Fixed
 - Fixed an issue which disabled persistent anchors in projects built on Windows machines.

## [1.2.0-pre.1] - 2025-10-10

### Added
 - Added a Dynamic-link library (DLL) build of the Unity OpenXR: Android XR plugin which can be found at `Runtime > Plugins > Windows > x64 > libUnityARFoundationAndroidXR.dll`.

### Changed
- Updated ARF package dependency to 6.4.0-pre.1

### Fixed
- Restored XR Management package dependency (4.5.0) to ensure manifest changes are configured correctly for GameActivity.

## [1.1.0-pre.2] - 2025-10-05

### Fixed
- Fixed the trailing space issue with "android.hardware.xr.input.controller " in AndroidManifest

## [1.1.0-pre.1] - 2025-08-06
